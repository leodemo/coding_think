...
def weighted_sampling_without_normalization(population, weights, seed=None):
    if seed is not None:
        random.seed(seed)
...
weights = [0.2, 0.3, 0.5, 0.05, 0.05]  # 权重总和不为1
seed = 42  # 随机种子，用于结果可复现

sample = weighted_sampling_without_normalization(population, weights, seed)
